...
///
/// These files are NOT bundled with MogHouse. They're GPLv3 in the
...
/// <c>MOGHOUSE_FFXI_RES</c> environment variable.
/// </summary>
public sealed class FfxiHuffmanTables
{
    ...
    {
        string? fromEnvironment = Environment.GetEnvironmentVariable("MOGHOUSE_FFXI_RES");
    ...
    private static uint[] ReadWords(string path)
    {
        if (!File.Exists(path))
        {
            throw new FileNotFoundException(
                $"FFXI compression table not found: {path}. These tables aren't bundled with MogHouse - " +
                $"point MOGHOUSE_FFXI_RES at a directory containing {EncodeFileName} and {DecodeFileName}.",
                path);
        }
