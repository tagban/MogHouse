...
// P/Invoke boundary matching native/moghouse_interop/include/moghouse_interop.h.
// NOT YET VERIFIED AGAINST A REAL NATIVE LIBRARY - moghouse_interop hasn't
// been built yet (see native/moghouse_interop/README.md). Signatures here
...
/// <summary>
/// Managed wrapper around a native MogHouse engine instance. The native
/// engine owns its own main-loop thread once <see cref="Run"/> is called
/// (mirroring lotus-ffxi's own usage - see native/moghouse_interop/README.md),
/// so <see cref="Run"/> must be invoked from a dedicated background thread,
/// never from a UI thread.
/// </summary>
public sealed class MogHouseEngine : IDisposable
{
    ...
    {
        if (GCHandle.FromIntPtr(userData).Target is MogHouseEngine engine)
        {
