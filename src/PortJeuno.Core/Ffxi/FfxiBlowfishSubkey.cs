...
/// <summary>
/// The initial Blowfish P-array (18 words) + S-boxes (4x256 words) FfxiBlowfish's
/// key schedule XORs the session key into - these are the standard, well-known
/// Blowfish constants (the first 32 bits decode to 0x243F6A88, matching the
/// published P[0] value from Schneier's original algorithm; verified against the
/// first 18 words directly). Stored pre-serialized as little-endian bytes,
/// mirroring how LandSandBoat's own reference implementation stores them, so the
/// exact same bytes feed FfxiBlowfish.Init on both sides.
/// </summary>
internal static class FfxiBlowfishSubkey
{
