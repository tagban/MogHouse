# PortJeuno C++/C# interop - design notes
...
own thread**; `PortJeunoGame : public FFXIGame` (see `src/portjeuno_interop.cpp`)
...
1. **CMake target linkage - the real blocker.** `ffxi-engine/ffxi/CMakeLists.txt`
   ...
   `ffxi-engine` can currently link against `FFXIGame`; `target_link_libraries(portjeuno_interop PUBLIC ffxi-lib)`
...
3. **Threading/marshaling on the C# side.** The tick callback fires on the
   ...
   designed beyond the P/Invoke signatures in `PortJeuno.Core/Interop/NativeEngine.cs`.
